...
local cb=false
local db={setShadow=function(_c,ac)cb=ac;_c:updateDraw()return _c end,getShadow=function(_c)return cb end,draw=function(_c)
bb.draw(_c)
_c:addDraw("shadow",function()
if(cb~=false)then local ac,bc=_c:getSize()
if(cb)then
...
bd:setDrawState("label",true)end;bd:updateDraw()end;return bd end,setBackground=function(bd,cd)
cc.setBackground(bd,cd)if(dc>1)then bd:setFontSize(dc)end;return bd end,setForeground=function(bd,cd)
cc.setForeground(bd,cd)if(dc>1)then bd:setFontSize(dc)end;return bd end,getFontSize=function(bd)return
...
bcb.timerId=os.startTimer(0.05)else bcb.finished()dbb:doneHandler(acb)end end end end}return cbb end}end;aa["libraries"]={}
...
b_a:listenEvent("mouse_scroll")b_a:listenEvent("mouse_drag")end,removeChildren=function(b_a)
bc.removeChildren(b_a)b_a:listenEvent("mouse_scroll")end,setParent=function(b_a,c_a,...)
bc.setParent(b_a,c_a,...)dc=c_a;return b_a end,scrollHandler=function(b_a,c_a,d_a,_aa)
if
...
return true end end end;__a(b_a,c_a)b_a:clearFocusedChild()return true end end,draw=function(b_a)
bc.draw(b_a)
...
if
(bc.dragHandler(__a,a_a,b_a,c_a))then if(bd)then local d_a=1;local _aa=1;d_a,_aa=dc:getAbsolutePosition()__a:setPosition(b_a+_d-
(d_a-1),c_a+ad- (_aa-1))
__a:updateDraw()end;return true end end,mouseHandler=function(__a,a_a,b_a,c_a,...)
if
...
table.remove(bc,_ca)return true end end;cba:removeEvents(dba)ad=false end
local function aaa(cba)local dba=cba:getParent()bc={}cc={}ad=false;bd=0;cd=0;_d=nil;if dba~=nil then
dba:removeEvents(cba)end end
...
local caa=b_a:getParent()caa:clearFocusedChild()end end,eventHandler=function(b_a,c_a,d_a,...)
_c.eventHandler(b_a,c_a,d_a,...)if(c_a=="paste")then
b_a:setValue(b_a:getValue()..d_a)end end,draw=function(b_a)
_c.draw(b_a)
...
local function baa()local _ba={}
for cba,dba in
pairs(colors)do if(type(dba)=="number")then
_ba[math.log(dba,2)]={term.nativePaletteColor(dba)}end end;local aba=ad.getMetadata("palette")if(aba~=nil)then
for cba,dba in pairs(aba)do _ba[cba]=dba end end
local bba=bd.getFrameData("palette")
if(bba~=nil)then for cba,dba in pairs(bba)do _ba[cba]=dba end end;return _ba end;local function caa()
...
#ad.getFrames()end,getActiveFrame=function(_ba)return __a end,loadImage=function(_ba,aba)
if
(fs.exists(aba))then local bba=ab.loadImageAsBimg(aba)ad=bb(bba)__a=1
bd=ad.getFrameObject(1)cd=ad.createBimg()dd=bd.getImage()caa()
_ba:updateDraw()end;return _ba end,setPath=function(_ba,aba)return
...
ac=ad;return _d end,getBaseSize=function(_d)return bc,cc end,getBaseWidth=function(_d)return bc end,getBaseHeight=function(_d)return cc end,setSize=function(_d,ad,bd,cd,dd)
...
local bd=false
local cd=ab({getBaseHeight=function(d_a)return 0 end,getBaseWidth=function(d_a)return 0 end,getPosition=function(d_a)return 0,0 end,getSize=function(d_a)return 0,0 end,isType=function(d_a)return
...
if aba==cd then
aaa=aaa+_aa+bc;caa=0;_aa=1;daa=daa+1;ad[daa]={offset=aaa}else
local bba=
ac=="row"and aba:getBaseWidth()or aba:getBaseHeight()
if(bba+caa<=baa)then table.insert(ad[daa],aba)
caa=caa+bba+bc else aaa=aaa+_aa+bc;_aa=ac=="row"and aba:getBaseHeight()or
aba:getBaseWidth()daa=daa+1
caa=bba+bc;ad[daa]={offset=aaa,aba}end
local cba=ac=="row"and aba:getBaseHeight()or aba:getBaseWidth()if cba>_aa then _aa=cba end end end end end
...
local cca=
dba:getFlexBasis()~=0 and dba:getFlexBasis()or dba:getBaseWidth()if caa>0 then _ca=cca+aca/caa*aba else _ca=cca end;if aba<0 and
...
local _ca;local aca=dba:getFlexGrow()local bca=dba:getFlexShrink()
local cca=
dba:getFlexBasis()~=0 and dba:getFlexBasis()or dba:getBaseHeight()if caa>0 then _ca=cca+aca/caa*aba else _ca=cca end;if aba<0 and
...
if(dd~=nil)then dd:addEvent(b_a,a_a)if(b_a==
"mouse_drag")then dd:addEvent("mouse_click",a_a)
dd:addEvent("mouse_up",a_a)end end;ad:registerEvent(b_a,c_a)
...
local daa=colors[caa]cd[math.log(daa,2)]=...
__a.setPaletteColor(daa,...)elseif(type(caa)=="table")then
for daa,_ba in pairs(caa)do cd[daa]=_ba
if(type(_ba)=="number")then __a.setPaletteColor(
2 ^daa,_ba)else local aba,bba,cba=table.unpack(_ba)__a.setPaletteColor(
2 ^daa,aba,bba,cba)end end end end;return baa end,setSize=function(baa,...)
...
for caa,daa in pairs(cd)do if(type(daa)=="number")then
__a.setPaletteColor(caa^2,daa)else local _ba,aba,bba=table.unpack(daa)
__a.setPaletteColor(caa^2,_ba,aba,bba)end end;cc.setMainFrame(baa)return baa end,render=function(baa)
if(
dc.render~=nil)then
...
if(ac=="horizontal")then __a:addText(ad,1,bc:rep(bd))
...
bc:setBackground(false)local dc=false;local _d=true;local ad,bd="Label","left"
local cd={getType=function(dd)return cc end,getBase=function(dd)return bc end,setText=function(dd,__a)
ad=tostring(__a)
if(_d)then local a_a=bb(ad,#ad)local b_a,c_a=1,1;for d_a,_aa in pairs(a_a)do c_a=c_a+1
b_a=math.max(b_a,_aa:len())end;dd:setSize(b_a,c_a)_d=true end;dd:updateDraw()return dd end,setForeground=function(dd,__a)
bc.setForeground(dd,__a)dc=true;return dd end,getAutoSize=function(dd)return _d end,setAutoSize=function(dd,__a)_d=__a;return dd end,getText=function(dd)return
ad end,setSize=function(dd,__a,a_a)bc.setSize(dd,__a,a_a)_d=false;return dd end,getTextAlign=function(dd)return
bd end,setTextAlign=function(dd,__a)bd=__a or bd;return dd end,draw=function(dd)bc.draw(dd)
dd:addDraw("label",function()
local __a,a_a=dd:getSize()
local b_a=

bd=="center"and math.floor(__a/2 -ad:len()/2 +0.5)or bd=="right"and __a- (ad:len()-1)or 1;cb(dd,b_a,1,ad,__a+1,a_a)end)end,init=function(dd)
bc.init(dd)local __a=dd:getParent()if not(dc)then
dd:setForeground(__a:getForeground())end end}cd.__index=cd;return setmetatable(cd,bc)end end
...
#bba))end else
b_a:addBlit(1,n+1,bba,bb[caa[n+dc].fgCol]:rep(#bba),bb[caa[n+dc].bgCol]:rep(
#bba))end end end end end)end}a_a.__index=a_a;return setmetatable(a_a,_c)end end;return aa["main"]()